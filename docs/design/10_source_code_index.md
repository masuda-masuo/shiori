...
*   **Allowlists and Glob Filtering**: Indexing is guarded by configuration keys: `SHIORI_DEV_REPOS` (which repos get code indexed; default empty = no code), `SHIORI_CODE_EXTENSIONS`, and `SHIORI_CODE_EXCLUDE_GLOBS`. `SHIORI_INDEX_CODE` is **deprecated** (issue #286) — when set to `true` without `SHIORI_DEV_REPOS`, all repos are treated as dev. Use `SHIORI_DEV_REPOS=owner/name1,owner/name2` instead. Folders containing dependencies or builds (e.g. `node_modules`, `.venv`, `dist`) are ignored. Files in unsupported languages fall back to plain-text line-limit chunking.
