...
def raw_fingerprint(raw):
    """Return first line of raw text, stripped of leading whitespace, truncated to 60 chars."""
...
def reg(fn, jp_fp, en_body):
    """Register translation. jp_fp: first line of raw text, stripped, ~60 chars."""
    key = (fn, jp_fp)
    EN[key] = tr(en_body)
...
reg('mcp_server.py', '"""意味ベースの検索（入口ツール）。言い換え・概念・クロスリンガルに強い。',
...
reg('mcp_server.py', '"""PR の変更ファイルマップ（メタデータ）を返す。ポインタ層のツール（issue #54, #100）。',
    'PR change file map (metadata pointer tool; issue #54, #100). Returns head_sha + file list. Content via GitHub MCP or shiori_read_pr_file.\n')
...
reg('mcp_server.py', '"""docs / issue/PR / code を GitHub から同期し索引を更新する（差分同期なので通常は数秒',
...
reg('search.py', '"""結果リストを指定されたキーと順序でソートする（後方互換ラッパー）。',
    'Sort result list by specified key and order (backward compatibility wrapper).\n')
reg('search.py', '"""キーワード検索（日本語対応トークナイズ）。関数名・API 名・エラーコード・設定キーなど',
    'Keyword search (Japanese-aware tokenization). Strong for exact matches: function names, API names, error codes, config keys.\n')
