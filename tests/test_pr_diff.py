...
class TestPrChangesIncludeDiff:
    ...
    def test_include_diff_true_returns_diff(self):
        ...
        mock_git_fetch = MagicMock(return_value="refs/shiori/tmp-abc")
        mock_git_delete = MagicMock()
        mock_git = MagicMock(return_value="diff --git a/src/a.py b/src/a.py\n@@ -1,3 +1,4 @@\n+new line")
        ...
        with (
            ...
            patch("shiori.mcp_server._git_fetch_ref", mock_git_fetch),
            patch("shiori.mcp_server._git_delete_ref", mock_git_delete),
            patch("shiori.mcp_server._git", mock_git),
            patch("shiori.mcp_server.settings") as mock_settings,
            patch("os.path.isdir", return_value=True),
            patch("os.path.realpath", return_value="/data/repos/o/r"),
        ):
    ...
    def test_include_diff_true_cleans_up_tmp_ref_on_error(self):
        ...
        mock_git_fetch = MagicMock(return_value="refs/shiori/tmp-abc")
        mock_git_delete = MagicMock()
        mock_git = MagicMock(side_effect=RuntimeError("git diff failed"))
